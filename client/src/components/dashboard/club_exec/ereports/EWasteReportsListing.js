...
import { getERListing, updateEReport } from '../../../../actions/formActions';
...
export default function EWasteReportsListing() {//This is a shorter method of defining a component and exporting it to be used (all in one)
  ...
  //Once the 'Set As Collected' button is clicked
  const updateRecord = e => {
    
    //The component captures the name and report id of the respective record in which the button was clicked
    const recName = e.target.name;
    const recId = e.target.value;   

    //Once the user selects 'OK' on  the confirmation box
    if(window.confirm("Are you sure you want set this E-Waste report by " + recName + " as collected.")){

      //Executes the updateEReport function from the formActions file in the actions folder using their report id
      //This essentially changes an e-waste report's status from 0 (Pending) to 1 (Collected)
      dispatch( updateEReport(recId) );

      //Notifies the user of successful update
      window.alert("E-Waste Report record updated successfully!");

      //This captures the updated e-waste reports from our backend database
      dispatch(getERListing());
        ...
        <React.Fragment>{/* The Fragment element is used to indicated that the following is a fragment/block of elements to be rendered by React */}
            ...
            <Grid item xs={12}>
              ...
              <Paper className={classes.paper}>
                  ...
                  <Title>E-Waste Report Listing</Title>
                      ...
                      <TableBody>
                            ...
                            {ereports ? ereports.map((record) => (
                                ...
                                <TableRow key={record.id}>
                                  ...
                                  {/* Set As Collected Button*/}
                                  <TableCell align="center"> 
                                        <Button outline color="success" onClick={updateRecord}  value={record.id} name={record.rep_person}>
                                            Set As Collected
                                        </Button>
                                  </TableCell>
